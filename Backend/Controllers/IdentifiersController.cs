...
namespace CoastalPharmacyCRUD.Controllers
{
    ...
    public class IdentifiersController : ControllerBase
    {
        ...
        {
            var categories = await _context.CDL_Identifiers
            .Where(c => c.Set == "CAT")
            .Select(c => new
            {
                c.Id,
        ...
        {
            var subCategories = await _context.CDL_Identifiers
            .Where(c => c.Set == "SAT")
            .Select(c => new
            {
                c.Id,
        ...
        {
            if (identifierDto == null) return BadRequest("Upss, there is no data here.");

            if (identifierDto.ParentId != null)
            {
                var existsParent = await _context.CDL_Identifiers.AnyAsync(p => p.Id == identifierDto.ParentId);
                if (!existsParent) return BadRequest("Upss, your information is not valid.");
            }
